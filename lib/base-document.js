...
class BaseDocument {
    ...
    static populate(docs, fields) {
        ...
        for (const key in anInstance._schema) {
            // Only populate specified fields
            if (isArray(fields) && fields.indexOf(key) < 0) continue
        ...
        for (const key in ids) {
            ...
            .then(function(dereferences) {
                ...
                for (const k in ids[key]) {
                    ...
                    for (const idIndex in ids[key][k]) {
                        const id = ids[key][k][idIndex]

                        // TODO: Replace with dereferences.find when able
                        // Find the right dereference
                        let deref;
                        for (const dIndex in dereferences) {
                            const d = dereferences[dIndex]
    ...
    _toData(keep) {
        ...
        for (const k in this) {
            if (k[0] === '_') {
                ...
                values[k] = [];
                for (const index in that[key]) {
                    values[k].push(that[key][index]._toData());
                }
            } else {
                values[k] = that[k];
            }
